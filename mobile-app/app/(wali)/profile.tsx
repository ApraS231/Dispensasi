  ...
  return (
    <View style={styles.container}>
      <SafeAreaView style={styles.safeArea}>
        <TopAppBar showAvatar={false} title="Profil Wali Kelas" showNotification={true} />

        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          
          <View style={styles.profileHeader}>
            <View style={styles.avatarWrapper}>
              <AvatarInitials name={user?.name || 'Wali'} size={100} fontSize={40} />
            </View>
            <Text style={styles.name}>{user?.name || 'Nama Wali Kelas'}</Text>
            <Text style={styles.email}>{user?.email || 'wali@sekolah.com'}</Text>
            <View style={styles.roleBadge}>
              <Text style={styles.roleText}>{user?.role?.replace(/_/g, ' ') || 'Wali Kelas'}</Text>
            </View>
          </View>
