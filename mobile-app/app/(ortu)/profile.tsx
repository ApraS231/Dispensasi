  ...
  return (
    <View style={styles.container}>
      <SafeAreaView style={styles.safeArea}>
        <TopAppBar showAvatar={false} title="Profil Orang Tua" showNotification={true} />

        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          
          <View style={styles.profileHeader}>
            <View style={styles.avatarWrapper}>
              <AvatarInitials name={user?.name || 'Ortu'} size={100} fontSize={40} />
            </View>
            <Text style={styles.name}>{user?.name || 'Nama Orang Tua'}</Text>
            <Text style={styles.email}>{user?.email || 'ortu@email.com'}</Text>
            <View style={styles.roleBadge}>
              <Text style={styles.roleText}>{user?.role?.replace(/_/g, ' ') || 'Orang Tua'}</Text>
            </View>
          </View>
          ...
          <SoftCard style={styles.menuCard}>
            <Text style={styles.sectionTitle}>Pemantauan Anak</Text>
            <OptionMenu icon="A" label="Kelola Data Anak" onPress={() => {}} />
            <View style={styles.divider} />
            <OptionMenu icon="L" label="Unduh Laporan Semester" onPress={() => {}} />
          </SoftCard>
