  ...
  return (
    <View style={styles.container}>
      <SafeAreaView style={styles.safeArea}>
        <TopAppBar showAvatar={false} title="Profil Saya" showNotification={true} />

        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          
          <View style={styles.profileHeader}>
            <View style={styles.avatarWrapper}>
              <AvatarInitials name={user?.name || 'User'} size={100} fontSize={40} />
            </View>
            <Text style={styles.name}>{user?.name || 'Nama Siswa'}</Text>
            <Text style={styles.email}>{user?.email || 'email@siswa.com'}</Text>
            <View style={styles.roleBadge}>
              <Text style={styles.roleText}>{user?.role?.replace(/_/g, ' ') || 'Siswa'}</Text>
            </View>
          </View>
          ...
          <SoftCard style={styles.menuCard}>
            ...
            <OptionMenu 
              icon="P" 
              label="Edit Profil & Pengaturan" 
              onPress={() => expoRouter.push('/profile-settings' as any)} 
            />
          ...
          <SoftCard style={styles.menuCard}>
            <Text style={styles.sectionTitle}>Bantuan & Info</Text>
